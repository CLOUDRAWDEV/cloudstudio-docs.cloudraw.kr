...
sidebar_position: 1
tags: [Azure, リソース, Cosmosdb, CosmosdbGremlin]
---
...
Azure Cosmos DB Gremlin は、Microsoft Azure が提供する完全管理型グラフデータベースサービスです。  
Gremlin はユーザーが Azure Cosmos DB でグラフデータを探索してクエリできるグラフ巡回言語です。  
頂点とエッジの生成、プロパティの追加と削除、複雑なグラフ巡回の実行を含む、幅広いグラフ操作をサポートします。
...
-   `gremlin_database`
    ...
    -   `throughput` : Gremlin データベースの処理量 - `400`~ `1000000`
...
-   `gremlin_graph`
    ...
    -   `partition_key_path` : パーティションキー
    -   `throughput` : Gremlin グラフのスループット - `400`~ `100000`
    -   `automatic` : 自動インデックスポリシーを使用するように設定 - `true`, `false`
    -   `indexing_mode` : インデックスタイプの設定 - `consistent` , `lazy`, `none`
    -   `included_paths` : インデックスに含めるパスのリスト
    -   `excluded_paths` : インデックスから除外されたパスのリスト
    -   `conflict_resoulution_mode` : コンフリクトレゾリューションモード - `LastWriterWins` , `Custom`
    -   `unique_key_paths` : ユニークキーに使用するパスのリスト
    -   `enable_autoscale` : Gremlin データベースのスループット - `true`, `false`
    -   `included_gremlin_database_name` : Gremlin グラフが作成される Gremlin データベースの名前
