...
- `managed_rule_set`
    - `type`: The type of rule set for protecting against vulnerabilities, such as `OWASP` or `Microsoft_BotManagerRuleSet`.
    - `version`: The version of the rule set.

- `managed_rule_group`
    - `type`: The type of rule set, such as `OWASP` or `Microsoft_BotManagerRuleSet`.
    - `rule_group_name`: The name of the rule group to be used.
    - `disabled_rules`: A list of rule IDs to be disabled.

- `exclusion`
    - `match_variable`: The variable to match, such as query strings or request methods, like `RequestArgNames`, `RequestCookieNames`, `RequestHeaderNames`.
    - `selector`: The value to compare with the matched variable using an operator.
    - `selector_match_operator`: The operator used to evaluate the match, such as `Contains`, `EndsWith`, `Equals`, `EqualsAny`, `StartsWith`.
