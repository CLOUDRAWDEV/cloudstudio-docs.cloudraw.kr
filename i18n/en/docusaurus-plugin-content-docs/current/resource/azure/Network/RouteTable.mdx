...
-   `disable_bgp_route_propagation`: Disable gateway route propagation to prevent on-premises route propagation to the subnet's network interface - `true`, `false`
-   `included_subnet_name`: The name of the subnet to associate with the route table
-   `route`
    -   `route_address_prefix`: The IP prefix representing the route
    -   `next_hop_type`: The type of next hop to handle packets that match the route - `VirtualNetworkGateway`, `VnetLocal`, `Internet`, `VirtualAppliance`, `None`
