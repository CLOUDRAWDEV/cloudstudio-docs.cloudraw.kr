...
title: KeyVault
sidebar_position: 0
tags: [Azure, 리소스, KeyVault]
---

import icon from "../img/key_vault-key_vault.png"
...
- ```key_vault_certificate``` 
  ...
  + ```exportable``` : Key의 외부 저장 활성화  - ```true```, ```false```
  + ```key_type``` : Key 타입  - ```RSA```, ```EC```
  + ```key_size``` : Key 사이즈 지정, Key 타입이 RSA인 경우 - ```2048/3072/4096```, Key 타입이 EC인 경우 - ```256/384/521```
  + ```curve``` : Key 타입이 EC인 경우 Curve 지정 - ```P-256```,```P-256K```,```P-384```,```P-521```
  + ```reuse_key``` : Key의 재사용 활성화 - ```true```, ```false```
