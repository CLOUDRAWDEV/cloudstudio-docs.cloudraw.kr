...
import icon from "../img/load_balancer-public_load_balancer.png"
...
- ```public_lb_frontend_ip_config``` 
  + ```pip_name``` : Public Load Balancer PIP의 이름
  + ```pip_sku``` : Public Load Balancer PIP의 SKU - ```standard```, ```Basic```
  + ```pip_allocation_method``` : Public Load Balancer PIP의 Allocation Method - ```Static```, ```Dynamic```
