...
크레덴셜을 미리 등록해두는 이유는 이후 캔버스를 배포할 때와 기존 인프라를 가져올 때 크레덴셜 정보가 필요하기 때문입니다.
...
<br />
11. 왼쪽 사이드 바에서 `인증서 및 암호`를 클릭합니다. 8. `새 클라이언트 암호`를 클릭합니다.
12. Key에 대한 간단한 설명을 입력한 후 만료시간을 선택합니다. 입력이 완료되면 `추가`를
클릭합니다. 10. 생성된 값을 복사하여 클라우드 스튜디오 Service Principal Key에 붙여넣기
합니다.
