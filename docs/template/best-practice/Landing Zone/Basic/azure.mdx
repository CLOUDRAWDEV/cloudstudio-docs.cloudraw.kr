...
> - ```Network Rules.IP_Rules```: [ "사용자 IP 설정" ]
...
>- ```Key Vault Secret.Activation Date```: 임의지정
...
> - ```Service Name```: cloudrawsql.com
> - ```Vnet Link```:resourceGroup:Application_vnet

> - ```Service Name```: cloudrawsa.com
> - ```Vnet Link```:resourceGroup:Application_vnet
...
> - ```Linked Connection Resource Name```: cloudrawmysql
> - ```Subresource Names```: ["mysqlServer"]

<caption caption="[그림.31] Azure Landing Zone Basic 31" src={azure31} />

> - ```Dns Zone Group Name```: cloudrawsql.com
> - ```Private Dns Zone Names```: cloudrawsql.com
> - ```Connection Name```: connect_cloudrawsql
...
> - ```SourceImage.Offer```: UbuntuServer
...
> - ```Disk Size Gb```: 127
> - ```Linked Network Interface Names```: 위에서 생성한 NIC와 연결
...
> - ```Backend Pool.Linux Virtual Machine Names```: LinuxVm:LInuxVmNic:ipconfig1
