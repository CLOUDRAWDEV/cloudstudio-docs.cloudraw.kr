...
> - ```Retention Day```: 365
> - ```Linked Storage Account Name```: HubRG:hubsa
...
> - ```Service Name```: hubsc
> - ```Linked Log Analytics Workspace```: HubRG:hublaw
...
이 가이드에서는 Admin ID,ssh public key를 사용해서 ubuntuVm을 배포합니다.
...
> - ```Network Interface Ip Config Name```: ubuntuVm-nic-config
> - ```Included Subnet Name```: platform_vnet:ApplicationSubnet
> - ```Pip Enable```: false
...
> - ```Network Interface Ip Config Name```: ubuntuVm-nic-ipconfig
...
> - ```Service Name```: platformsc
> - ```Linked Log Analytics Workspace```: PlatformRG:platformlaw
> - ```Assessment Policy.Name```: default_assessment_policy
> - ```Assessment Policy.Severity```: Medium
...
> - ```Assessment.Name```: default_assessment
> - ```Assessment.Policy Name```: default_assessment_policy
> - ```Assessment.Target Resource Name```: PlatformRG:platformlaw
> - ```Assessment.Status Code```: Healthy
...
> - ```Service Name```
> - ```Peering1.Name``` : vnp686351-peering1
...
> - ```Peering2.Name``` : vnp686351-peering2
...
> - ```Linked Resource Group Name```: HubRG
