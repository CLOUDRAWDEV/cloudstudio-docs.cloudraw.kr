...
> - ```Service Name```: etc_ubuntu_sn
> - ```Included Vnet Name```: etc_ubuntu_vn
...
> - ```Ssh Public Key```: 본인이 생성한 ssh public key 값을 넣는다
...
> - ```Included Subnet Name```: etc_ubuntu_vn:etc_ubuntu_sn
> - ```Pip Enable```: False
> - ```Pip Sku```: Basic
