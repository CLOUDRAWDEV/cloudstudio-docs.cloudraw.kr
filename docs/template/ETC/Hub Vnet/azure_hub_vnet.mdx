...
### 4. Firewall 생성
**Firewall** 아이콘을 가져옵니다.
Firewall이 배포될 Subnet은 AzureFirewallSubnet으로 고정된 이름을 가집니다. 
Firewall을 생성할 떄 Subnet이 같이 배포되도록 구성되어 있습니다. 
Address Prefixes는 AzureFirewallSubnet이 사용할 CIDR을 입력합니다.
Included Vnet Name은 Firewall 아이콘을 Virtual Network에 넣으면 자동으로 입력됩니다.
...
Virtual Network Gateway가 배포될 Subnet은 GatewaySubnet으로 고정된 이름을 가집니다. 
Virtual Network Gateway를 생성할 떄 Subnet이 같이 배포되도록 구성되어 있습니다. 
Address Prefixes는 GatewaySubnet이 사용할 CIDR을 입력합니다.
Virtual Network Gateway를 정상적으로 배포하는데 30 ~ 40정도 소요됩니다.
Virtual Network Gateway를 사용하기 위해서는 배포 후, On-premise의 VPN 디바이스와 연결해야 합니다.
