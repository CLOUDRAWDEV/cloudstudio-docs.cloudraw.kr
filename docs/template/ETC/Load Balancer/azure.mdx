...
> - ```Linked Subnet Names```: etclbvn:lb_subnet
...
> - ```Service Name```: etclbvm1,2,3
> - ```Size```: Standard_D2s_v3

> - ```Enable Ssh Key Authentication```: true
> - ```Admin Username```: cloudraw
> - ```Ssh Public key```: 본인이 생성한 ssh public key값을 넣는다
...
> - ```Included Subnet Name```: etclbvn:lb_subnet
> - ```Pip Enable```: false
...
> - ```Service Name```: lb_bh
> - ```Included Subnet Name```: etclbvn:AzureBastionSubnet
> - ```Sku```: Basic
...
> - ```Public Lb Frontend Ip Config.Pip Name```: lbipconfig-pip
...
> - ```Backendaddress1``` 
>   + ```Virtual Network Name``` : etclbvn
>   + ```Linked Virtual Machine Name``` : ubuntuVm-nic-ipconfig:etclbvm1

> - ```Backendaddress2``` 
>   + ```Virtual Network Name``` : etclbvn
>   + ```Linked Virtual Machine Name``` : ubuntuVm-nic-ipconfig:etclbvm2

> - ```Backendaddress3``` 
>   + ```Virtual Network Name``` : etclbvn
>   + ```Linked Virtual Machine Name``` : ubuntuVm-nic-ipconfig:etclbvm3

<caption caption="[그림.22] Azure Load Balancer 22" src={azure22} />

> - ```Backend Pool.Name```: backendpool
> - ```Backend Pool.Backend Address Names```: Backendaddress1,Backendaddress2,Backendaddress3
...
> - ```Service Name```: lb_ng
> - ```Linked Subnet Name```: etclbvn:lb_subnet
> - ```Pip Name```: ng-pip
...
<caption caption="[그림.25] Azure Load Balancer 20" src={azure25} />
