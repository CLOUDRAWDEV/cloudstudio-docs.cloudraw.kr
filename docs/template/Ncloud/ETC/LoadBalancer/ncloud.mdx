...
<hr />
<br/>
Load Balancer는 수신 트래픽을 다수의 서버로 분산시키는 서비스로, 등록된 Target Group의 서버로 수신 트래픽을 분산시켜 가용성을 높이고 시스템 가동률을 조절하는 역할을 수행합니다. 
이로 인해 워크로드의 가용성을 향상시켜 예기치 못한 서버의 장애 또는 예정된 변경 작업 등에 대하여 중단 없이 대응할 수 있도록 지원합니다.
...
크레덴셜이 없는 경우 [**크레덴셜등록**](https://docs.cstudio.app/docs/credential/createCredential#ncloud-%ED%81%AC%EB%A0%88%EB%8D%B4%EC%85%9C-%EB%93%B1%EB%A1%9D%ED%95%98%EA%B8%B0)를 확인해주시기 바랍니다.
